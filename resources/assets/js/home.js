...
function getDependencies(width) {
    ...
    if (BrowserDetect.browser == 'Explorer' && BrowserDetect.version < 10) {
        return {animations: 'homeanimations'}
    }
...
function resizeFunctions(oldWidth, newWidth, dimension) {
    ...
    if (findBreakpoint(oldWidth) != findBreakpoint(newWidth)) {
        ...
        if (newWidth <= 768) {
            ...
            require(['require', 'homeanimations'], function (require) {
                var animations = require('homeanimations');
                animations.initialize();
            });

        } else {

            require(['require', 'homeanimations@md'], function (require) {
                var animations = require('homeanimations@md');
        ...
        media.resize(newWidth, dimension);
...
function init() {
    ...
    console.log(deps.animations);

    // Require all the dependencies we need, and initialise them
    require(['positioning', 'media', deps.animations], function (positioning, animations, media) {
